...
class GoogleAPIAuthenticator:
    ...
    """
    def __init__(self, scopes: list[str], key_path: str | None = None, credentials_file: str | None = None, token_file: str | None = None, project_id: str | None = None):
        """
    ...
    def authenticate(self):
        ...
        if not self.creds or not self.creds.valid:
            if os.path.exists(self.credentials_file):
                print(f"Attempting authentication using client secrets file: {self.credentials_file}")
                try:
                    flow = InstalledAppFlow.from_client_secrets_file(self.credentials_file, self.scopes)
                    self.creds = flow.run_local_server(port=0)
                    with open(self.token_file, 'w') as token:
                        token.write(self.creds.to_json())
                    print(f"Successfully authenticated and saved token to '{self.token_file}'.")
                except Exception as e:
        ...
        if not self.creds:
            print("No specific credentials provided or valid. Falling back to Application Default Credentials.")
            try:
    ...
    def get_storage_client(self) -> storage.Client:
        ...
        self.authenticate()
        if not self.project_id:
            print("Warning: Project ID could not be determined. You may need to specify it manually for GCS.")
...
if __name__ == '__main__':
    ...
    try:
        ...
        auth_gcs = GoogleAPIAuthenticator(scopes=gcs_scopes, key_path=SERVICE_ACCOUNT_FILE, project_id=MY_PROJECT_ID)
