...
def rearrange_folders(source_dir, dest_dir):
    ...
    for resource_type in ['video', 'audio']:
        for gender in ['male', 'female']:
            for lang_code in set(language_map.values()): # Creates eng, spa, etc.
                 # Also create a generic 'spa' folder as per the request, even if no source files map to it yet.
                os.makedirs(os.path.join(base_voiceover_path, resource_type, 'gender', gender, 'language', 'spa'), exist_ok=True)
                os.makedirs(os.path.join(base_voiceover_path, resource_type, 'gender', gender, 'language', lang_code), exist_ok=True)
            ...
            if len(path_parts) > 1:
                ...
                # --- Handle Audio and Video Files ---
                    ...
                    if len(path_parts) >= 5: # source_root/Audio/Female/Ara/file.m4a
                        ...
                        lang_code = language_map.get(lang_folder, 'unknown') # Default to 'unknown' if not in map

                        if lang_code != 'unknown':
                            dest_folder = os.path.join(base_voiceover_path, resource_type, 'gender', gender, 'language', lang_code)
                            new_filename = filename.lower()
                            dest_path = os.path.join(dest_folder, new_filename)
                            print(f"Moving {source_file_path} to {dest_path}")
                            shutil.move(source_file_path, dest_path)
                        else:
                            print(f"Warning: No language mapping for folder '{lang_folder}' in '{source_file_path}'. Skipping file.")
                    else:
                        print(f"Warning: File '{source_file_path}' is in an unexpected subdirectory. Skipping.")
...
if __name__ == '__main__':
    source_root = input('type the /path/to/your/original/folder: ')  # e.g., 'C:/Users/YourUser/Desktop/Original'
    destination_root = input('type the /path/to/your/new/folder: ') # e.g., 'C:/Users/YourUser/Desktop/Rearranged'
    ...
    if os.path.isdir(source_root):
        ...
        print("Please create a dummy folder and files or update the path to your actual source folder.")